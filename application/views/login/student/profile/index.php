<?php if($this->session->flashdata('update_stu')) : ?>
    <?php echo '<p class="alert alert-success mt-3">' .$this->session->flashdata('update_stu') . '</p>'; ?>
<?php endif; ?>
<button type="button" class="btn btn-primary mt-4" id="update">Update Your Details</button><hr>
<style>
#alert{
	color:red;
}
...
$(function(){
	...
	$('#register').click(function(){
		...
		if(first_name=="" || first_name=="0"){ $('.first_name').addClass('is-invalid first_name'); return false; }
			...
			var idd=<?= $student->id?>;
			$.ajax({
				url:"<?= base_url('student/profile/update/"+idd+"')?>",
				type:'post',
				dataType:'json',
				async:true,
...
<?= form_open('',['id'=>'register_form']) ?>
	...
	<h3 id="tag">Educational Details</h3>
	<div class="row">
		<div class="col-sm-12">
			<div class="form-group">
	...
	<div class="row">
		<div class="col-sm-6">
			<div class="form-group">
				...
				<select class="form-control from" id="from" name="from" readonly>
		...
		<div class="col-sm-6">
			<div class="form-group">
				...
				<select class="form-control to" name="to" id="to" readonly>
	...
	<br><input type="button" value="Submit" id="register" class="btn btn-primary" disabled>
<?= form_close() ?>